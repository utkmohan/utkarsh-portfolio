...
- **Links and navigation:** edit the named constants near the top of `build.mjs`. `BASE` must remain `/utkarsh-portfolio/` unless the repository name changes. Google tracking wrappers were resolved to their original destinations. The unrelated empty MIT biography link is explicitly disclosed, not silently corrected.
